...
router.post('/verify-password', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const validPassword = await require('bcryptjs').compare(password, userResult.rows[0].password_hash);
...
router.post('/secrets', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const key = deriveKey(password, user.encryption_salt);
        const secretsResult = await pool.query('SELECT * FROM two_factor_secrets WHERE user_id = $1', [userId]);
